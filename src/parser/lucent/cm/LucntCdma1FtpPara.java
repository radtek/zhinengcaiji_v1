...
/**
 * 目前只适用于上海，  原始文件中有表头，通过表头找到与模板字段的对应关系
 * @author linp  20150717
 *
 */
public class LucntCdma1FtpPara extends Parser {
	...
	public boolean parseData() throws Exception {
		...
		try {
			brh = new BufferedReader(new InputStreamReader(new FileInputStream(fileName)));
			String lineh = null;
			while ((lineh = brh.readLine()) != null) {
				String[] lhead = lineh.split(pub.separator);
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for(int j = 0; j < head.size(); j++){
				if(head.get(j).equalsIgnoreCase(list.get(i).src)){
					//sqlldr中txt的第i行对应原始文件中的第j行
					map.put(i, j);
				}
			}
		}
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				sbd.append(omcid).append(";").append(collecttime).append(";").append(stamptime).append(";");
				for (int i = 0; i < list.size(); i++) {
					if(map.get(i) != null){
						sbd.append(sp[(Integer) map.get(i)] != null ? sp[(Integer) map.get(i)].trim() : "");
					}else{
						sbd.append("");
					}
					sbd.append(";");
				}
				txtPW.println(sbd.substring(0, sbd.length()-1));
				txtPW.flush();
			}

			txtPW.close();
			String cmd = "sqlldr userid=%s/%s@%s skip=%s control=%s bad=%s log=%s errors=9999999 bindsize=20000000 rows=5000 readsize=20000000";
