 ...
 */
public class JiangXiAlarmParser extends Parser {
	...
	// 由于厂家问题，每隔段时间都要重新建立一次连接。
	private static final int RECONNECTION_TIME = 60 * 60 * 1000;
	...
	public boolean parseData() throws Exception {
		...
		try {
			...
			log.debug(taskId + "[" + getCollectObjInfo().getDescribe() + "] 监听端口已开启，等待对方连接：" + server);
	...
	static private class Work extends Thread {

		private JiangXiAlarmParser parser = null;

		private Socket client;

		public Work(JiangXiAlarmParser parser, Socket client) {
			this.parser = parser;
			this.client = client;
		}
		...
		public void run() {
			...
			try {
				...
				log.debug(parser.taskId + " 接收到了一个客户端连接：" + client);
