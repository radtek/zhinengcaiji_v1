 ...
 * 湖北全量终端数据。
 * 
 * @author ChensSijiang 2013-4-23
 */
public class HuBeiUeFully extends Parser {

	public static void main(String[] args) throws Exception {
		DevInfo di = new DevInfo();
		CollectObjInfo task = new CollectObjInfo(808);
		task.setLastCollectTime(new Timestamp(Util.getDate1("2013-04-23 00:00:00").getTime()));
		task.setDevInfo(di);
		HuBeiUeFully parser = new HuBeiUeFully();
		parser.setCollectObjInfo(task);
		parser.fileName = "F:\\资料\\原始数据\\湖北终端-全量\\R01_TYZD_SERV_ESN_MSG_201301.dat";
		...
		try {
			...
			while (lines.hasNext()) {
				...
				try {
					...
					String vendor = HuBeiVendorMap.find(sp[7].trim());
					String model = sp[8].trim();
