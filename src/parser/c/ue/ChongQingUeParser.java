...
/**
 * 重庆终端数据，从优网自己的MOD_PHONE_MODEL表里取的。这里是导成CSV再解析。
 * 
 * @author ChensSijiang 2013-4-28
 */
public class ChongQingUeParser extends Parser {

	public static void main(String[] args) throws Exception {
		ChongQingUeParser p = new ChongQingUeParser();
		p.fileName = "C:\\Users\\ChensSijiang\\Desktop\\MOD_PHONE_MODEL.csv";
		CollectObjInfo task = new CollectObjInfo(1);
		task.setLastCollectTime(new Timestamp(Util.getDate1("2013-04-28 00:00:00").getTime()));
	...
	public boolean parseData() throws Exception {
		...
		try {
			...
			lines = IOUtils.lineIterator(in, "utf-8");
			String line = null;
			String sp[] = null;
			int count = 0;
			...
			while (lines.hasNext()) {
				...
				try {
					...
					String vendor = sp[1].trim();
					if (Util.isNull(vendor))
						continue;
					String model = sp[2].trim();
