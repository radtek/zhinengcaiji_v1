...
public class GdTemp extends Parser {
	...
	public boolean parseData() throws Exception {
		...
		try {
			...
			while (lines.hasNext()) {
				...
				try {
					...
					if (Util.isNull(vendor) || Util.isNull(model))
						continue;
		...
		File bakImsiIndex = new File(target.getAbsolutePath() + ".history." + Util.getDateString_yyyyMMddHHmmss(new Date()));
		if (target.exists()) {
			// IMSI_INDEX.zip已存在，备份。
			if (target.renameTo(bakImsiIndex)) {
				log.debug("已将文件+" + target + "备份为" + bakImsiIndex);
			} else {
				log.warn("将文件+" + target + "备份为" + bakImsiIndex + "时失败。");
			}
		}
		...
		if (ZipUtil.zipDir(srcDir, target, false)) {
			...
			if (bakImsiIndex.exists()) {
				if (!bakImsiIndex.renameTo(target))
					log.warn("将备份的" + bakImsiIndex + "还原成" + target + "失败。");
			}
		}
	...
	public static void main(String[] args) throws Exception {
		GdTemp gd = new GdTemp();
		gd.fileName = "F:\\资料\\原始数据\\广东终端\\TREM_PART05.csv";
		CollectObjInfo task = new CollectObjInfo(9999);
		task.setLastCollectTime(new Timestamp(0));
		DevInfo di = new DevInfo();
		task.setDevInfo(di);
		gd.setCollectObjInfo(task);
		gd.parseData();
	}
