...
/**
 * 针对江苏电信2.5期扩容做的，在电信的JSTELECOM.T_SA_TERMINAL表里采集数据，此类只负责生成文件（给ILAP用）， 采集入库由数据库方式采集来采。
 * 
 * @author ChenSijiang 2012-8-23
 */
public class JSDbUEInfoParser extends Parser {
	...
	void handleRs() throws Exception {
		...
		while (rs.next()) {
			...
			try {
				...
				String termmodel = rs.getString("termmodel");
