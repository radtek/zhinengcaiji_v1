 ...
 */
public class PowerfulTokenizer implements Enumeration<String> {
	...
	public String nextToken() {
		...
		else if ((sToken.trim().startsWith("\"")) && (!((sToken.trim().endsWith("\"")) && (!sToken.trim().endsWith("\"\""))))) {
	...
	public int countTokens() {
		...
		if (bIncludeDelim) {
			...
			while (iIdx != -1) {
				if (!((sInput.charAt(iIdx - 1) == '"') && (sInput.charAt(iIdx + 1) == '"') && ((iIdx + 1 + iLen <= sInput.length()) && (sInput
						.substring(iIdx + 1, iIdx + 1 + iLen).equals(sDelim))))) {
					iTokens--;
				}

				// don't decrement the token count if consecutive tokens
				// are found.
				while ((iIdx + 1 < sInput.length()) && (sInput.substring(iIdx + 1, iIdx + 1 + iLen).equals(sDelim))) {
					iIdx += iLen;
				}
				iIdx = sInput.indexOf(sDelim, iIdx + 1);
			}
	...
	/**
	 * Checks whether the particular index (at which the delimiter is found is within double quotes (i.e. in a substring). This also checks whether
	 * the token is equal to the delimeter.
	 * 
	...
	private boolean isWithinQuotes(int k) {
		...
		if (!bIncludeDelim) {
			...
			if ((sInput.charAt(k - 1) == '"') && (sInput.charAt(k + 1) == '"')
					&& ((k + 1 + iLen <= sInput.length()) && (sInput.substring(k + 1, k + 1 + iLen).equals(sDelim)))) {
				iTokens -= 2;
				return false;
			}
		}
	...
	public static void main(String[] args) {
		...
		/*
		 * for (int i=0; i<iTokens; i++) { System.out.println("token" + i + " = " + oMT.nextToken()); } oMT = new PowerfulTokenizer(sIn, ",");
		 */
		int i = 0;
