...
public class DBFWriter extends DBFBase {
	...
	public DBFWriter(File dbfFile) throws DBFException {
		...
		try {
			...
			/*
			 * before proceeding check whether the passed in File object is an empty/non-existent file or not.
			 */
			...
			switch (this.header.fieldArray[j].getDataType()) {
				...
				case 'D' :
					if (objectArray[j] != null) {
						...
						dataOutput.write(Utils.textPadding(String.valueOf(calendar.get(Calendar.MONTH) + 1), this.characterSetName, 2,
								Utils.ALIGN_RIGHT, (byte) '0'));
						dataOutput.write(Utils.textPadding(String.valueOf(calendar.get(Calendar.DAY_OF_MONTH)), this.characterSetName, 2,
								Utils.ALIGN_RIGHT, (byte) '0'));
					} else {
				...
				case 'F' :
					...
					if (objectArray[j] != null) {
						...
						dataOutput
								.write(Utils.textPadding("?", this.characterSetName, this.header.fieldArray[j].getFieldLength(), Utils.ALIGN_RIGHT));
					}
